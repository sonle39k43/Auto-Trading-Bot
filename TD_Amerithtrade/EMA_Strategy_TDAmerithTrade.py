        ...
        try:
            ...
            for bar in msg['content']:
                ...
                if minutes_diff > 0 and math.floor(minutes_diff) % self.barsize == 0:
                    ...
                    if len(self.bars) > 0:
                        #Calculate EMA50 on bar close
                        self.ema50 = ta.trend.ema_indicator(self.close_array,50,True)
                        self.ema200 = ta.trend.ema_indicator(self.close_array,200,True)

                        #If EMA50 > EMA200 and we are not in position then we buy
                        # 7 day volume get min, and if today's volume is > min
                        if self.ema50[len(self.ema50)-1] > self.ema200[len(self.ema200)-1] \
                            self.currentBar.volume > self.min_vol and not self.inPositionL:
                            order = tda.orders.equities.equity_buy_market(self.symbol,1)
                            self.inPosition = True

                        #If EMA50 <= EMA200 and we are in Position => Sell
                        if self.ema50[len(self.ema50)-1] <= self.ema200[len(self.ema200)-1] and self.inPosition:
                ...
                self.min_vol  = min(self.bars[-7:-1])
